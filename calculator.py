...
if st.button("🔍 Calculate", use_container_width=True):
    ...
    # Display the results with markdown for better formatting
    st.markdown('<h3 style="color:white;">Results</h3>', unsafe_allow_html=True)
    st.markdown(f'<p style="color:white; font-size:16px;"><strong>Total bandwidth required:</strong> {bandwidth:.2f} {output_unit}</p>', unsafe_allow_html=True)
    st.markdown(f'<p style="color:white; font-size:16px;"><strong>Total rejected bags ({time_unit}):</strong> {rejected_bags:.2f} bags</p>', unsafe_allow_html=True)
